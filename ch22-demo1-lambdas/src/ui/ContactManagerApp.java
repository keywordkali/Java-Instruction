...
//p702-705
		...
		contactsWithoutPhone = filterContacts(contacts, c -> c.getPhone() == null); // c is argument name that is
	...
	public static List<Contact> filterContacts(List<Contact> contacts, TestContact condition) {
		...
		for (Contact c : contacts) {
			if (condition.test(c)) { // condition . test can be used for any variable
